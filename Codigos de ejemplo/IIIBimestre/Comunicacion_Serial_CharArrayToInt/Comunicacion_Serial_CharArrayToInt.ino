 ...
 * Carnet: ---------
 * Proyecto: Comunicacion serial - recepcion de una cadena de caracteres y convertida a una variable entera
*/
...
void setup()
{
  ...
  pinMode(led, LOW);
  Serial.println("Clase de comunicacion serial - recepcion de una cadena de caracteres y conversion de esta a entero");
}
...
void loop() 
{
  ...
  if(Serial.available()>0) //verificamos que hayan datos en el buffer serial
  {
    ...
    //Si la informacion que he recibido corresponde a una variable numerica, tengo que convertirla a un entero 
    ...
    {  
      //Si recibido entre el numero 10 y el numero 20
      Serial.println("Ha ingresado un valor entre 11 y 19");  //Envio el texto al monitor serial.
    }else
    {
      Serial.println("He ingresado un valor fuera del rango establecido");
    }
